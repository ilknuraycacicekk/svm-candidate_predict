...
def transform_data_num(df):
    ...
    for col in data.columns:
        if col=="candidate_id" or col=="Final":
            pass 
        else :
...
df = df.drop(columns=["candidate_id","Technic_Result"])
print(df.columns)

X = df.drop(columns=["Final"])  # 'Final' sütununu bağımsız değişkenlerden çıkar
y = df["Final"]  # 'Final' sütununu hedef değişken olarak ayarla
...
print("Doğruluk Skoru: ", accuracy_score(y_test, y_pred)) #0.95
